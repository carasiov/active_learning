...
mpl.rcParams.update(
    {
        ...
        # Include bundled fonts (e.g. DejaVu) so Matplotlib always finds a fallback
        "font.sans-serif": [
            "Inter",
            "Circular Std",
            "Helvetica",
...
mpl.rcParams["axes.prop_cycle"] = cycler(color=ACCENT_CYCLE)

LATENT_POINT_SIZE = 18
CHANNEL_POINT_SIZE = 12
...
def style_axes(
    ...
    *,
    ...
    if title:
        ax.set_title(title, fontweight="semibold", fontsize=13, color="#111827")
    if xlabel:
        ax.set_xlabel(xlabel, fontsize=11, color="#1F2937")
    if ylabel:
        ax.set_ylabel(ylabel, fontsize=11, color="#1F2937")
