...
def configure_jax_device():
    ...
    try:
        ...
        if "CUDA_ERROR_NO_DEVICE" in str(exc) or "No device found" in str(exc):
            logger.warning(f"GPU initialization failed: {exc}")
            ...
            return "cpu"
        logger.error(f"JAX initialization failed: {exc}")
        raise
