...
class LatentSpaceViewer:
    def __init__(self, data):
        self.path_labels = './data/labels.csv'
        ...
        self.plots[f'latent_space'] = ax.scatter(self.scatter_plot_coords[:,0], self.scatter_plot_coords[:,1], s=3)
        ...
        # Radio-Buttons für Coloring
        rax = plt.axes([0.0, 0.0, 0.12, 0.08])  # Position der Buttons
        self.radio = RadioButtons(rax, ('user labels',                                        
                                        'pred_class',
                                        'true_class',
                                        'certainty'), active=0)
    ...
    def update_colors(self):
        ...
        # Für die anderen Fälle: Clippen, Normalisieren und Farben zuweisen
    ...
    def on_scatter_click(self,event):
        ...
        if event.inaxes != self.axes['latent']:
            return  # Beendet die Funktion, falls nicht im Scatterplot geklickt wurde.

        try:
            print(f"Clicked {event.button} at (x,y)=({event.xdata:.2f},{event.ydata:.2f})")
        except TypeError:
            # Will be raised if click outside axis, because xdata/ydata then is None.
            return

        if not event.button==1: #left mouse-click
            return

        # if interactive tool like panning/zooming is active, do nothing
